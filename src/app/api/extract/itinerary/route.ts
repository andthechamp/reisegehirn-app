...
import { ITINERARY_EXTRACTION_SYSTEM_PROMPT } from "@/lib/prompts";
import { parseExtractedItinerary } from "@/lib/itinerary-schema";
import {
  findUnsupportedFile,
...
export async function POST(req: NextRequest) {
  try {
    ...
    const response = await anthropic.beta.messages.create({
      ...
      max_tokens: 4000,
      system: ITINERARY_EXTRACTION_SYSTEM_PROMPT,
      ...
      messages: [
        {
          ...
          content: [
            ...
            {
              type: "text",
              text: "Extrahiere den Reiseverlauf (Tage, Häfen, Zeiten) aus diesem Dokument gemäß dem vorgegebenen JSON-Schema.",
            },
          ],
        },
      ],
    ...
    const port_calls = parseExtractedItinerary(textBlock.text);
    console.log("Reiseverlauf-Extraktion Ergebnis:", JSON.stringify(port_calls));
    return NextResponse.json({ port_calls });
  } catch (err) {
    console.error("Reiseverlauf-Extraktion fehlgeschlagen:", err);
    const message =
