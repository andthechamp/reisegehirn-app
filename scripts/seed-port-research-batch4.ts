// Einmaliges Seed-Skript für eine vierte Charge extern recherchierter
// Hafeninformationen (Nordamerika-Ostküste, Bermuda, Azoren-Zwischenstopp
// Horta), NICHT Teil der laufenden App. Format entspricht exakt dem
// ResearchFinding-Schema aus der KI-Websuche-Recherche (siehe
// port-research.ts/research-schema.ts). Rohdaten liegen in
// scripts/data/port-research-batch4.json - Struktur/Logik 1:1 aus
// seed-port-research-batch3.ts übernommen.
//
// Aufruf:
//   node --env-file=.env.local scripts/seed-port-research-batch4.ts
//
...
const dataPath = path.join(__dirname, "data", "port-research-batch4.json");
const findings: RawFinding[] = JSON.parse(readFileSync(dataPath, "utf-8"));

// Keine Kollisionen mit bereits kanonisierten Häfen aus Batch 2/3 gefunden -
// alle Titel dieser Charge sind eigenständige, neue Häfen.
